 ...
 * @LastEditTime : 2023-09-03 01:22:01
  ...
  <div class="md:flex flex-col md:flex-row min-h-screen w-full shadow bg-black pt-14 pb-16">
    <div class="w-full px-4 pt-2">
      ...
      <div class="mt-4 md:flex">
        <AlbumCard v-for="collection in collections" :key="collection.id" :name="collection.title"
          :cover_url="collection.cover" max_width="md:max-w-md" @click.native="toPlaylist(collection.id)">
        </AlbumCard>
      </div>
      <!-- finals -->
      <HeaderTitle title="年终精选" @click.native="pushToPlaylistDetail('finals')"></HeaderTitle>
      <div class="mt-4 md:grid md:grid-cols-5 md:gap-4 flex flex-wrap">
        <AlbumCard v-for="final in finals" :key="final.id" :name="final.title" :cover_url="final.cover"
      ...
      <div v-show="netease_daily.length !== 0">
        ...
        <div class="mt-4 md:grid md:grid-cols-5 md:gap-4 flex flex-wrap">
          <div
            ...
            <h5 class="mb-2 text-xs font-medium overflow-hidden whitespace-nowrap overflow-ellipsis">
              每日歌曲推荐
            </h5>
          </div>
          <AlbumCard v-for="playlist in netease_daily" :key="playlist.id" :name="playlist.title"
            :cover_url="playlist.cover" @click.native="toPlaylist(playlist.id, 'netease-daily')">
          </AlbumCard>
        </div>
      </div>
      <!-- local albums -->
      ...
      <div class="mt-4 md:grid md:grid-cols-5 md:gap-4 flex flex-wrap">
        <AlbumCard v-for="album in albums" :key="album.id" :name="album.title" :cover_url="album.cover"
      ...
      <div class="mt-4 md:grid md:grid-cols-5 md:gap-4 flex flex-wrap">
        <AlbumCard v-for="list in netease_toplist" :key="list.id" :name="list.title" :cover_url="list.cover"
          @click.native="toPlaylist(list.nId, 'netease')">
        </AlbumCard>
      </div>
      <HeaderTitle title="热门新碟" @click.native="pushToPlaylistDetail('albums')"></HeaderTitle>
      <div class="mt-4 md:grid md:grid-cols-5 md:gap-4 flex flex-wrap">
        <AlbumCard v-for="album in netease_topalbums" :key="album.id" :name="album.name" :cover_url="album.picUrl"
          :artist="album.artist.name" @click.native="toPlaylist(album.id, 'netease-album')">
        </AlbumCard>
      </div>
    </div>
    <loading :active="isLoading" :can-cancel="true" :is-full-page="true" background-color="rgba(0, 0, 0, 0.5)"
      color="#EC4899" />
  </div>
</template>
...
const collections = ref([]) as [Playlist]
const finals = ref([]) as [Playlist]
const albums = ref([]) as [Playlist]
const netease_toplist = ref([]) as [Playlist]
// router
...
const netease_topalbums = ref([]) as [Netease_Album]
